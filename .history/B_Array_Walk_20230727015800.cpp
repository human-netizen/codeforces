...
ll pref[100005];
ll ans;
void rec(string cur , ll remk , ll rem , ll res , ll idx){
    ...
    if(cur.back() != 'l')rec(cur + "l" , min(remk - 1 , rem) , rem , res , idx);
    rec(cur + "r" , remk - 1 , rem , res , idx);
}
ll main(){
    tc{
        ll n , rem , k;cin>>n>>rem>>k;
        for(ll i = 1 ; i <= n ; i++)cin>>ara[i];
        for(ll i = 1 ; i <= n ; i++)pref[i] = pref[i - 1] + ara[i];
        ll sum = 0;
        for(ll i = 1 ; i <= n ; i++){
            sum+=ara[i];
            rec("" , min(k , rem - i + 1) , rem - i + 1 , sum , i);
        }
        cout<<ans<<endl;
        ans = 0;
    }
